...
def readadc_temp(adcnum, clockpin, mosipin, misopin, cspin):
    if((adcnum > 7) or (adcnum < 0)):
        return -1
    GPIO.output(cspin, True)
...
def ledshine(time):
    for i in range(2*time):
        adc_value = readadc(light_ch, SPICLK, SPIMOSI, SPIMISO, SPICS, output_pin, output_pin2)
        print(adc_value)
        if(i % 2 == 0):
            GPIO.output(output_pin, True)
            GPIO.output(output_pin2, True)
...
def main(argv):
    ...
    if(argv[1][1] == 'i'):
        #print("are you ok?")
        adc_value = readadc(light_ch, SPICLK, SPIMOSI, SPIMISO, SPICS, output_pin3, output_pin4)
        print(adc_value)
        return adc_value
    elif(argv[1][1] == 'e'):
        #print("are you ok?")
        adc_value = readadc_temp(temp_ch, SPICLK, SPIMOSI, SPIMISO, SPICS)
        print(adc_value)
        return adc_value
    elif(argv[1][1] == 'E'):
        #print("elif")
